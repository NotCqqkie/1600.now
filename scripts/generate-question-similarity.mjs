...
const pastQuestionDifficultyMap = readJson("src/data/pastQuestionDifficultyMap.json");
...
const resolveDifficulty = (record) =>
  record.bankType === "past"
    ? normalizeDifficultyLabel(pastQuestionDifficultyMap[String(record.question.id)]) ?? normalizeDifficultyLabel(record.question.difficulty)
    : normalizeDifficultyLabel(record.question.difficulty);
...
const output = `// Question similarity data. Regenerate with scripts/generate-question-similarity.mjs.
